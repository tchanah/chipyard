...
static void fill_node_chunk_data(uint32_t node,
    ...
    for (int i = 0; i < NUM_ELEMENTS; i += 2) {
        uint16_t val0 = float_to_raw16(dst_f[i]);
        uint16_t val1 = float_to_raw16(dst_f[i + 1]);
...
int main(int argc, char *argv[]) {
    ...
    for (int test_set = 0; test_set < NUM_TEST_SETS; test_set++) {
        ...
        // Calculate expected Level 4 output simulating the exact tree reduction
        for (int chunk = 0; chunk < total_chunks; ++chunk) {
            
            // Retrieve all 8 nodes' Level 0 data
            static float all_nodes_data[NUM_NODES][NUM_ELEMENTS];
            for (int node = 0; node < NUM_NODES; ++node) {
                fill_node_chunk_data(node, chunk, all_nodes_data[node], temp_node_words);
            }

            // Simulate the exact recursive doubling tree to match hardware associative rounding
            for (int level = 1; level <= MAX_RECURSION_LEVEL; ++level) {
                int distance = 1 << (level - 1);
                static float next_level_data[NUM_NODES][NUM_ELEMENTS];
                for (int node = 0; node < NUM_NODES; ++node) {
                ...
                // Copy next_level_data back to all_nodes_data for the next iteration
                for (int node = 0; node < NUM_NODES; ++node) {
                    for (int i = 0; i < NUM_ELEMENTS; ++i) {
            ...
            // Expected Level 4 result is what this node (TEST_NODE_RANK) ends up with
            for (int i = 0; i < NUM_ELEMENTS; ++i) {
...
#else
            for (int i = 0; i < NUM_ELEMENTS; i += 2) {
                uint16_t val0 = float_to_raw16(expected_outputs_f[MAX_RECURSION_LEVEL][chunk][i]);
                uint16_t val1 = float_to_raw16(expected_outputs_f[MAX_RECURSION_LEVEL][chunk][i + 1]);
                expected_outputs[MAX_RECURSION_LEVEL][chunk][i / 2] = ((uint32_t)val1 << 16) | (uint32_t)val0;
            }
#endif
        }
        ...
        // --- Process Ring Buffers (Initialize on test set 0 only) ---
        // For subsequent test sets, the buffers are already posted and the 
        // hardware pointer is preserved, so we MUST preserve software rx_tail.
        if (test_set == 0) {
        ...
        while (responses_received < total_expected_responses) {
            ...
            if (nic_recv_comp_avail() > 0) {
                ...
                // For 16-bit: 256 words, each contains 2 packed 16-bit elements
                {
                    ...
                    if (bad_word >= 0) {
                        printf("ERROR: Level 4 payload mismatch for chunk %u!\n", response_chunk_index);
